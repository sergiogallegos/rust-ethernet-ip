...
- **One usability finding (filed as polish, not blocking):** the C# runner resolves the manifest path relative to the current working directory. Running `dotnet run` from `examples/CSharpFullCoverage/` fails with `DirectoryNotFoundException`. Workaround: invoke from the repo root via `dotnet run --project examples/CSharpFullCoverage`. The Rust and Python runners have the same cwd-sensitivity; manifest-path resolution should ideally be relative to the script or via `--manifest` flag. Not a library bug; pure example-harness ergonomics.
