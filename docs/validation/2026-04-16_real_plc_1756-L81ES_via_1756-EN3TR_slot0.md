...
This is a follow-up validation for the `0.7.1` draft line against the same ControlLogix target and `gTest*` tag set used during the `0.7.0` release validation.
...
The Rust library remains stable on the exercised routed ControlLogix feature set for the `0.7.1` draft line. The observed failures are unchanged controller firmware limitations, not new protocol regressions.
