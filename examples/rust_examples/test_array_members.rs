...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for &index in &test_indices {
        ...
        match timeout(Duration::from_secs(5), client.read_tag(&tag_path)).await {
            ...
            Ok(Err(e)) => {
                ...
                if e.to_string().contains("Path segment error") || e.to_string().contains("CIP Error 4") {
    ...
    let start_index = 0;
    let count = 10;
    println!("  Reading {}[{}..{}]...\n", array_tag_name, start_index, start_index + count - 1);
    ...
    let array_types = vec![
        (format!("{}", array_tag_name), "Same array"),
        (format!("Program:MainProgram.{}", array_tag_name), "Program-scoped"),
    ];

    for (test_tag, description) in &array_types {
        let tag_path = format!("{}[0]", test_tag);
        print!("  Testing {} ({})... ", tag_path, description);
    ...
    match timeout(Duration::from_secs(5), client.write_tag(&tag_path, test_value.clone())).await {
        Ok(Ok(())) => {
    ...
    println!("\n📈 Test Summary");
    println!("===============");
    println!("  Total individual reads: {} successful, {} failed", success_count, fail_count);
    println!("  Range reads: {} successful, {} failed", range_success, range_fail);
    ...
    if fail_count == 0 && range_fail == 0 {
        ...
        println!("   If you see 'Path segment error' or 'CIP Error 4', the fix may need adjustment.");
    }
