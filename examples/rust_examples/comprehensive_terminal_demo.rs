...
use rust_ethernet_ip::{EipClient, PlcValue, RoutePath, UdtData, BatchOperation};
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    println!("╔════════════════════════════════════════════════════════════════╗");
    println!("║  🦀 Rust EtherNet/IP - Comprehensive Terminal Demo 1.0.0                ║");
...
async fn setup_connection(addr: &str) -> Result<(EipClient, bool), Box<dyn std::error::Error + Send + Sync>> {
...
async fn test_connection_management(
    ...
    if let Ok(detailed) = client.check_health_detailed().await {
        println!("  Detailed: {}", if detailed { "✅ OK" } else { "❌ Failed" });
    }

    println!("\n⚙️ Configuration:");
    let batch_config = client.get_batch_config();
    println!("  Batch Config: {} ops/packet, {} bytes max",
        batch_config.max_operations_per_packet,
        batch_config.max_packet_size);
...
async fn test_tag_discovery(
    ...
    match client.discover_tags().await {
        ...
        Err(e) => println!("⚠️ Basic discovery failed: {} (some PLCs don't support this)", e),
    }
    ...
    match client.discover_tags_detailed().await {
        Ok(tags) => {
            ...
            if !tags.is_empty() {
                ...
                for tag in tags.iter().take(5) {
                    ...
                    println!("      Type: {} ({})", tag.data_type_name, format!("0x{:04X}", tag.data_type));
        ...
        Err(e) => println!("⚠️ Detailed discovery failed: {} (some PLCs don't support this)", e),
    }
    ...
    match client.get_udt_definition(udt_name).await {
        Ok(definition) => {
            ...
            for member in &definition.members {
                println!("    - {}: Type=0x{:04X}, Offset={}, Size={}",
        ...
        Ok(PlcValue::String(value)) => {
            ...
            if !write_value.is_empty() && write_value != "skip" {
                ...
                match client.write_string(string_tag, write_value).await {
                    ...
                    Err(e) => println!("❌ Write failed: {} (Note: Some PLCs restrict STRING writes)", e),
                }
            }
        }
...
async fn test_batch_operations(
    ...
    match client.read_tags_batch(&test_tags).await {
        Ok(results) => {
            ...
            println!("  Throughput: {:.1} ops/sec", test_tags.len() as f64 / duration.as_secs_f64());
    ...
    match client.write_tags_batch(&write_tags).await {
        Ok(results) => {
            ...
            println!("  Throughput: {:.1} ops/sec", write_tags.len() as f64 / duration.as_secs_f64());
            for (tag, result) in results {
                match result {
    ...
    let operations = vec![
        BatchOperation::Read { tag_name: "TestTag".to_string() },
        BatchOperation::Read { tag_name: "TestBool".to_string() },
        BatchOperation::Write { tag_name: "TestInt".to_string(), value: PlcValue::Dint(999) },
    ];
...
async fn test_performance(
    ...
    let mut tag_input = String::new();
    io::stdin().read_line(&mut tag_input)?;
    let test_tag = if tag_input.trim().is_empty() { "TestTag" } else { tag_input.trim() };
    ...
    println!("  Average latency: {:.2} ms", duration.as_millis() as f64 / success as f64);
    ...
    let start = Instant::now();
    match client.read_tags_batch(&batch_tags.iter().map(|s| s.as_str()).collect::<Vec<_>>()).await {
        Ok(results) => {
            let duration = start.elapsed();
            let success_count = results.values().filter(|r| r.is_ok()).count();
            let ops_per_sec = success_count as f64 / duration.as_secs_f64();
            println!("✅ Completed: {} successful in {:?}", success_count, duration);
            println!("  Throughput: {:.1} ops/sec", ops_per_sec);
            println!("  Average latency: {:.2} ms", duration.as_millis() as f64 / success_count as f64);
            println!("  Speedup: {:.1}x faster than individual", ops_per_sec / (success as f64 / duration.as_secs_f64()));
        }
...
async fn test_health_monitoring(
    ...
    println!("  Max Operations per Packet: {}", config.max_operations_per_packet);
    ...
    println!("  Optimize Packet Packing: {}", config.optimize_packet_packing);

    Ok(())
}

fn parse_value(current: &PlcValue, input: &str) -> Result<PlcValue, Box<dyn std::error::Error + Send + Sync>> {
    match current {
        PlcValue::Bool(_) => Ok(PlcValue::Bool(input.parse::<bool>().unwrap_or(input == "true" || input == "1"))),
