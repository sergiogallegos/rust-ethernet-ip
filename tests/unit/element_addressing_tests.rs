...
fn test_tagpath_element_addressing_8bit() {
    ...
    assert!(found_element, "Path should contain 8-bit element segment (0x28)");
}
...
fn test_tagpath_element_addressing_16bit() {
    ...
    for i in 0..cip_path.len().saturating_sub(3) {
        if cip_path[i] == 0x29 {
            found_element = true;
            assert_eq!(cip_path[i + 1], 0x00, "16-bit segment should have padding byte");
            let index = u16::from_le_bytes([cip_path[i + 2], cip_path[i + 3]]);
            assert_eq!(index, 1000, "Element index should be 1000");
            break;
        }
    }
    assert!(found_element, "Path should contain 16-bit element segment (0x29)");
}
...
fn test_tagpath_element_addressing_32bit() {
    ...
    for i in 0..cip_path.len().saturating_sub(5) {
        if cip_path[i] == 0x2A {
            found_element = true;
            assert_eq!(cip_path[i + 1], 0x00, "32-bit segment should have padding byte");
    ...
    assert!(found_element, "Path should contain 32-bit element segment (0x2A)");
}
...
fn test_tagpath_element_addressing_boundaries() {
    ...
    assert!(cip_path_255.contains(&0x28), "Index 255 should use 8-bit segment");
    
    // Boundary: 256 (16-bit start)
    let path_256 = TagPath::parse("MyArray[256]").unwrap();
    let cip_path_256 = path_256.to_cip_path().unwrap();
    assert!(cip_path_256.contains(&0x29), "Index 256 should use 16-bit segment");
    ...
    assert!(cip_path_65535.contains(&0x29), "Index 65535 should use 16-bit segment");
    ...
    assert!(cip_path_65536.contains(&0x2A), "Index 65536 should use 32-bit segment");
}
...
async fn test_element_id_segment_8bit() {
    ...
    assert_eq!(segment, vec![0x28, 0x00], "Index 0 should use 8-bit segment");
    
    // Test index 5
    let segment = client.build_element_id_segment(5);
    assert_eq!(segment, vec![0x28, 0x05], "Index 5 should use 8-bit segment");
    
    // Test index 255 (max 8-bit)
    let segment = client.build_element_id_segment(255);
    assert_eq!(segment, vec![0x28, 0xFF], "Index 255 should use 8-bit segment");
...
async fn test_element_id_segment_32bit() {
    ...
    assert_eq!(segment, expected, "Index u32::MAX should use 32-bit segment");
    assert_eq!(segment.len(), 6, "32-bit segment should be 6 bytes");
}
...
async fn test_element_id_segment_boundaries() {
    ...
    assert_eq!(segment_65535[0], 0x29, "Index 65535 should use 16-bit segment");
    
    // Boundary: 65536 (32-bit start)
    let segment_65536 = client.build_element_id_segment(65536);
    assert_eq!(segment_65536[0], 0x2A, "Index 65536 should use 32-bit segment");
}
...
async fn test_build_read_array_request_single_element() {
    ...
    let path_start = 2;
    assert_eq!(request[path_start], 0x91, "Path should start with ANSI Extended Symbol (0x91)");
...
async fn test_build_read_array_request_16bit_index() {
    ...
    assert!(found_29, "Path should contain 16-bit element segment (0x29)");
}
...
async fn test_build_read_array_request_32bit_index() {
    ...
    assert!(found_2a, "Path should contain 32-bit element segment (0x2A)");
}
...
async fn test_build_base_tag_path() {
    ...
    assert_eq!(path[0], 0x91, "Path should start with ANSI Extended Symbol (0x91)");
...
async fn test_build_write_array_request_with_index() {
    ...
    assert_eq!(req_data_type, data_type, "Data type should be DINT (0x00C4)");
...
async fn test_path_word_alignment() {
    ...
    assert_eq!(path.len() % 2, 0, "Path should be word-aligned (even length)");
...
async fn test_element_id_segment_little_endian() {
    ...
    assert_eq!(segment[3], 0x12, "High byte should be second (little-endian)");
